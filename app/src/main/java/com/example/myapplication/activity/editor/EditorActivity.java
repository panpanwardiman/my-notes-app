...
public class EditorActivity extends AppCompatActivity implements EditorView {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        el_title = findViewById(R.id.title);
        el_note = findViewById(R.id.note);
    ...
    private void editMode() {
        el_note.setFocusableInTouchMode(true);
        el_note.setFocusableInTouchMode(true);
        palette.setEnabled(true);
    }

    private void readMode() {
        el_note.setFocusableInTouchMode(false);
        el_note.setFocusableInTouchMode(false);
        el_note.setFocusable(false);
        el_note.setFocusable(false);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        String title =  el_title.getText().toString().trim();
        String note =  el_note.getText().toString().trim();
        int color = this.color;

        switch (item.getItemId()) {
            case R.id.save:
                if (title.isEmpty()) {
                    el_title.setError("Please enter a title");
                } else if (note.isEmpty()) {
                    el_note.setError("Please enter note");
                } else {
